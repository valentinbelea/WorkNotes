...
- Referințele interne dintre note (`NoteReferenceRules`, `note-references.js`): textul paragrafului păstrează referința ca `[[note:{id}|{număr}]]`, legată prin ID-ul notei destinație, nu prin titlu; forma se schimbă numai împreună, în Business și în JavaScript. Se propun doar proprietarului notei, pentru un număr de 3–18 cifre găsit întreg în titlu, doar celelalte note ale aceluiași context pe care le poate vedea; nimic nu devine referință fără alegerea utilizatorului. `NoteReferences` se reface la salvarea textului, numai cu referințele valide; nu scrieți în ea din alte fluxuri. O referință care nu se mai poate deschide rămâne în text, marcată, fără titlul destinației.
