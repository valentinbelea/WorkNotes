...
- În timpul drag-ului, cardul preluat este estompat, cu contur punctat accent (note-cell--dragging). Celulele eligibile din aceeași lună au o nuanță verde foarte ușoară și un contur fin (note-cell--drop-eligible), iar celula de sub cursor, cu care s-ar face schimbul, o nuanță puțin mai puternică și conturul accent de 2px (note-cell--drop-target). Culorile sunt amestecuri ale tokenului --wn-primary cu transparent (color-mix), fără valori noi în paletă. Celulele altor luni nu primesc nicio clasă. La sfârșitul drag-ului (dragend: după drop sau la anulare, Escape ori drop în altă parte) toate clasele sunt eliminate.
- După drop, celulele își schimbă locurile în DOM imediat ce drag-ul se încheie (dragend); în timpul drag-ului nu se mută nicio celulă. Serverul salvează schimbul în fundal și răspunde cu ordinea lunii. Un drag nou poate începe imediat, și cât timp schimburile anterioare încă se salvează: salvările pleacă pe rând, în ordinea schimburilor, iar după ultimul răspuns fiecare listă urmează ordinea salvată de server. Dacă o salvare eșuează, apare mesajul de eroare (status-message--error) cu textul din .resx, iar schimbul respectiv se anulează: lista revine la ordinea salvată dinaintea lui. Un drop în altă lună este ignorat: browserul arată că nu este permis, iar cardul rămâne pe loc.
...
- Panoul are culoarea tipului notei active (note-sheet--journal / note-sheet--article) și conține: headerul (sigla, taburile, Minimizează, Închide) și, pentru fiecare tab, titlul editabil pe loc, editorul și footerul (bara de informații; tipul, contextul și vizibilitatea; starea salvării și Salvează).
