...
- **Referințe interne între note** (jurnale și articole): după un număr tastat în editor și terminat cu spațiu, punctuație, rând nou sau Tab, dacă numărul apare întreg în titlul altor note ale tablei pe care utilizatorul le poate vedea, o sugestie discretă sub el („Creează referință către”, cu tipul și titlul complet al fiecărei note) oferă transformarea în referință; nimic nu se schimbă fără alegerea utilizatorului (click, sau Tab, săgeți și Enter). Referința ține ID-ul notei (textul păstrează `[[note:{id}|{număr}]]`, editorul arată doar numărul, ca link cu tooltip) și deschide nota într-un tab nou sau îi selectează tabul existent; din previzualizarea cardurilor, și cu editorul minimizat. Referințele care nu se mai pot deschide rămân în text, marcate. Relațiile sunt păstrate în `NoteReferences` (scriptul 002).
